...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class MarketWaveStrategy : Strategy
    {
        ...
        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Time Threshold", Order = 4, GroupName = "Trade Logic")]
        public double TimeThreshold
        { get; set; }
       
        [NinjaScriptProperty]
        [Display(Name = "Time threshold greater than (true) or less than (false)", Order = 5, GroupName = "Trade Logic")]
        public bool longer
        {get; set;}
            ...
            protected override void OnMarketData(MarketDataEventArgs e)
        {
            ...
            if (Math.Abs(newPriceLevel - currentPriceLevel) >= TickSize)
            {
                ...
                if (newPriceLevel > currentPriceLevel)
                {
                    ...
                    if (!wasLastUp){
                        ...
                        oldLevelData.CurrentStreak+=e.Volume;        // continue incrementing if direction remains "up"
                ...
                else
                {
                    ...
                    if (wasLastUp){
                        ...
                        oldLevelData.CurrentStreak-=e.Volume;        // continue decrementing if direction remains "down"
            ...
            if (priceLevels.TryGetValue(currentPriceLevel, out var currentLevelData) && !tradeTaken)
            {
                ...
                if (  currentLevelData.CurrentStreak >= HighThreshold || currentLevelData.CurrentStreak <= LowThreshold )
                {
                    ...
                    if (orderId.Length > 0 || atmStrategyId.Length > 0 || Position.MarketPosition != MarketPosition.Flat)
                        return;
                    ...
                    //Print($"Trade Taken. Ask Volume: {currentLevelData.AskVolume}, Bid Volume: {currentLevelData.BidVolume}, Importance Score: {currentLevelData.ImportanceScore}, Time at level {currentLevelData.TimeSpent}, Streak: {currentLevelData.CurrentStreak}");
                       ...
                       if(State == State.Historical && Position.MarketPosition == MarketPosition.Flat){
                           if(currentLevelData.CurrentStreak <= LowThreshold  && Position.MarketPosition == MarketPosition.Flat ){
                               
                                EnterLong("Long");
                                SetProfitTarget(CalculationMode.Ticks, 20);
                                SetStopLoss(CalculationMode.Ticks,32);
                               tradeTaken = true;
                           } else if (currentLevelData.CurrentStreak >= HighThreshold  && Position.MarketPosition == MarketPosition.Flat){
                        ...
                        if (isLongMode && isTrendMode && currentLevelData.CurrentStreak <= LowThreshold )
                        {
                        ...
                        if (isLongMode  && currentLevelData.CurrentStreak <= LowThreshold && isRegressionMode )
                        {
                        ...
                        if (isShortMode &&  isTrendMode  && currentLevelData.CurrentStreak >= HighThreshold)
                        {
                        ...
                        if (isShortMode && currentLevelData.CurrentStreak >= HighThreshold && isRegressionMode )
                        {
